...
WITH real_and_theorical AS (
    ...
    FROM (
             ...
             SELECT DISTINCT bus_line_id,
                             ...
                             bus_stop_id,
                             'real' "type"
             FROM tcc_lucas_matches_2019_05_02_bus_line_020 c
    ...
    LIMIT 1
     ...
     LATERAL (
         SELECT CASE
                    ...
                    END AS bus_arrival_time,
                CASE
                    WHEN who = 'early' THEN r_early.arrival_time_delay
                    WHEN who = 'delayed'
                        THEN r_delayed.arrival_time_delay
                    END AS arrival_time_delay,
                CASE
