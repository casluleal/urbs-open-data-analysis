...
--------------- PURE TABLES ---------------
     veiculos AS (
         SELECT
             ...
             -- data do arquivo
             file_date
         FROM veiculos_2019_05_14_bus_line_216
     ),
     pontos_linha AS (
         SELECT
...
--------------- CREATE SHAPES AND AZIMUTHS ---------------
     shapes_and_azimuths AS (
         ...
         FROM (
                  ...
                  FROM shape_linha WINDOW W AS (PARTITION BY (file_date, bus_line_id, shp) ORDER BY id)
              ) AS q1
...
-- **************** VEICULOS WITH AZIMUTHS ****************
     veiculos_with_azimuth AS (
         SELECT file_date,
                bus_line_id,
                vehicle_id,
                LAG(timestamp) OVER                   w AS prev_dthr, timestamp,
                timestamp - LAG(timestamp) OVER       w time_dif, st_makeline(LAG(geom) OVER W, geom) AS trajectory_line,
     ...
     va_pa AS (
         SELECT va.file_date,
                ...
                MIN(l1.distance_bus_to_stop) OVER w_preceding min_distance_bus_to_stop_preceding, MIN(l1.distance_bus_to_stop) OVER w_following min_distance_bus_to_stop_following
         FROM veiculos_with_azimuth va -- O onibus e o ponto de onibus precisam ser da mesma linha
        	...
        	-- Calcula a proporção sobre a linha de trajetória do ponto geográfico onde o ônibus mais perto ficou do ponto de ônibus st_linelocatepoint(
                      va.trajectory_line, pa.bus_stop_point_geom) ratio_closest_point_vehicle_bus_stop
                  ) l0,
              lateral(
                  SELECT
                ...
                -- Calcula a diferença entre o azimute da trajetória e o azimute do ponto de onibus (
                  ...
                  -- Calcula a estimativa do momento passado pelo onibus
                                                                                                    va.prev_dthr +
                                                                                                    (va.time_dif * l0.ratio_closest_point_vehicle_bus_stop) bus_arrival_time
                  ) l1
     ...
     tabela_veiculo_window AS (
         SELECT *,
                LAG("time") OVER          w,
                "time" - LAG("time") OVER w dif_lag, LEAD("time") OVER w,
                LEAD("time") OVER         w - "time" dif_lead
         FROM tabela_veiculo WINDOW W AS (PARTITION BY bus_line_id, vehicle_id, bus_stop_id ORDER BY "time" ASC)
...
--AND c.bus_arrival_time BETWEEN tvb.left_bound AND tvb.right_bound 
     ...
     matches_theorical_report_count AS (
         SELECT bus_line_id,
                vehicle_id,
                COUNT(*)                                                   filter (WHERE "sum" = 1) eq_1, COUNT(*) filter (WHERE "sum" <> 1) dif_1, COUNT(*) total,
